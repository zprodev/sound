...
const _Sound = class {
   ...
   */
  preferUrl(urls) {
    const [{ url }] = urls.map((url2) => ({ url: url2, ext: core.utils.path.extname(url2).slice(1) })).sort((a, b) => supported.extensions.indexOf(a.ext) - supported.extensions.indexOf(b.ext));
    return url;
  }
